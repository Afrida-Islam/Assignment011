...
const PaymentSuccess = () => {
  ...
  if (!paymentDetails) {
    return (
        <div className="flex flex-col items-center justify-center p-10 bg-red-100 rounded-lg shadow-lg text-center">
            <h1 className="text-3xl font-bold text-red-600 mb-2">Payment Status Unknown</h1>
            <p className="text-gray-600 mb-6">
                We couldn't confirm your payment details. Please check your applications or contact support.
            </p>
            <Link
  ...
  const { scholarshipName, universityName, amountPaid, transactionId } = paymentDetails;
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
      <div className="bg-white p-10 max-w-lg w-full rounded-2xl shadow-2xl text-center border-t-8 border-green-500">
        ...
        <div className="border border-gray-200 p-4 rounded-lg bg-gray-50 space-y-3 mt-6 text-left">
          <h2 className="text-xl font-semibold border-b pb-2 text-gray-700">Order Summary</h2>
          
          <div className="flex justify-between">
            <span className="text-gray-500">Scholarship:</span>
            <span className="font-medium text-gray-800">{scholarshipName || 'N/A'}</span>
          </div>

          <div className="flex justify-between">
            <span className="text-gray-500">University:</span>
            <span className="font-medium text-gray-800">{universityName || 'N/A'}</span>
          </div>
        ...
        <Link
          to="/dashboard/my-applications"
          className="mt-8 inline-block w-full bg-lime-600 text-white font-semibold py-3 px-6 rounded-lg shadow-md hover:bg-lime-700 transition duration-300 transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-lime-300"
        >
          Go to My Applications
        </Link>
      </div>
